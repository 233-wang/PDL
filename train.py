...
def train_for_one_epoch(model, g_loss, discriminator_loss, train_loader, optimizer, epoch_number, args):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        # soft_label_shape = soft_label.shape
        # soft_no_softmax_shape = soft_no_softmax.shape
        # print("output shape:", output.size())
        # print("soft_label shape:", soft_label.size())
        # print("soft_no_softmax shape:", soft_no_softmax.size())
        g_loss_output = g_loss((output, soft_label), labels)
        # 假设 g_loss_output 是一个包含 (output, soft_label) 的元组
        # print("g_loss_output - output shape:", g_loss_output[0].size())
        # print("g_loss_output - soft_label shape:", g_loss_output[1].size())
